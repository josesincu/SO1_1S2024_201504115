...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Jose Castro Sincu");
MODULE_DESCRIPTION("So1 Proyecto 1 - Modulo CPU");
...
static int escribir_a_proc(struct seq_file *file_proc, void *v)
{
    ...
    for_each_process(task) {
        unsigned long cpu_time = jiffies_to_msecs(task->utime + task->stime); //Tiempo total del proceso
    ...
    seq_printf(file_proc, "\"num_cpu\":%d,\n", num_online_cpus());
